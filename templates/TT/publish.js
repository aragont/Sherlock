...
function generate(title, docs, filename, resolveLinks) {
   console.log("1",title, filename, resolveLinks);
    ...
    var outpath = path.join(outdir, filename),
        ...
        var tag_a = html.indexOf('<a href=\"'+filename);
        var temp = html.substr(tag_a, 100);
        var temp_1 = temp.replace('none','block');
        html = html.replace(temp, temp_1);
...
function buildMemberNav(items, itemHeading, itemsSeen, linktoFn) {
    var nav = '';
    console.log("3",items,itemHeading, itemsSeen, linktoFn);
    if (items.length) {
        var itemsNav = '';
...
function buildNav(members) {
    ...
    if (members.modules.length) {
        ...
        members.modules.forEach(function(g) {
            console.log(g.kind, ' ', g.name);
            if ( g.kind === 'module') {
                //console.log(g.name);
                globalNav += '<li onclick="show_functions(this)">' + linkto(g.longname, g.name) +'<ul style="display:none">';
                for (var i=0; i<members.modules.length; i++) {
                    var name_module = members.modules[i].longname.substr(members.modules[i].longname.indexOf(':')+1,members.modules[i].longname.indexOf('#') - members.modules[i].longname.indexOf(':')-1 );
                    if (members.modules[i].kind!=='typedef' && name_module === g.name) {
                        //console.log(members.modules[i].kind, name_module, members.modules[i].longname);
                        globalNav += '<li style="margin-left:20px">' + linkto(members.modules[i].longname, members.modules[i].name) + '</li>';
                    }
                }
                globalNav+='</ul></li>';
        ...
        if (!globalNav) {
            ...
            nav += '<h3>' + linkto('Sherlock', 'Sherlock') + '</h3>';
        }
        else {
            nav += '<h3>Sherlock</h3><ul>' + globalNav + '</ul>';
        }
    }
...
exports.publish = function(taffyData, opts, tutorials) {
    ...
    if (members.modules.length) {  generate('Module', [{kind: 'moduleobj'}], globalUrl); }
